...
def inner_boundary(Pc,Tc,M,mu,m,optable,epsilon):
    ...
    """
    rhoc = ldensity(P=Pc,T=Tc,mu=mu)
    r = inner_radius(rho=rhoc,m=m)
    l = np.power(10,m) * epsilon
...
def inner_radius(rho,m):
    ...
    :param m: Initial mass step from center of star.
    """
    return (1/3) * (np.log10(3/(4*np.pi)) + m - rho)
...
def inner_pressure(Pc,rho,m):
    ...
    Pdelt = np.log10((3*G)/(8*np.pi)) + (4/3) * (np.log10((4/3)*np.pi) + rho + (1/2)*m)
    return np.log10(np.power(10,Pc) - np.power(10,Pdelt))
