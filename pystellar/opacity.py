...
def read_table_opal(fkey,cfg):
    ...
    assert table_num == c["tables.num"], "Number of Tables must add up to configured value! Counted: %d, CFG: %d" % (table_num,c["tables.num"])
    ...
    for i in xrange(table_num):
        ...
        for line in table_list[c.get("table.head",0):-c.get("table.foot",0)]:
            line = line.rstrip("\n")
            if len(line) < char_line:
                line += "  ?????"*((char_line-len(line))/c["table.chars"])
            table_txt += line + "\n"
        table_txt += "".join(table_list[-c.get("table.foot",0):]) #Tack the footer rows back on
        ...
        table = np.genfromtxt(
            ...
            skip_footer=c.get("table.foot",0),
        )
        begin = end
    ...
    nrows = c["table.end"] - c["table.begin"] - c["table.head"] - c["table.foot"] - 1
...
class OpacityTable(object):
    ...
    def __init__(self, fkey, method='linear',load=True, filename="data/OPAL.yml", X=None, Y=None):
    ...
    def _composition_interpolator(self):
        """Creates the compositional interpolator"""
        ...
        log.debug("Input NaNs: [x,y]=%g, [v]=%g" % (np.sum(np.isnan(points)), np.sum(np.isnan(values))))
    ...
    def _temperature_density_interpolator(self):
        ...
        log.debug(u"Input NaNs: [logR,logT]=%g, [κ]=%g" % (np.sum(np.isnan(points)), np.sum(np.isnan(values))))
    ...
    def check_range(self,points):
        ...
        maxes = np.array([(points[:,i] < np.max(self._tbls[self.n,:,i])).any() for i in xrange(2)]).any()
        mines = np.array([(points[:,i] > np.min(self._tbls[self.n,:,i])).any() for i in xrange(2)]).any()
        if mines and maxes:
            return
        ...
        raise ValueError("BOUNDS: Error Index Unknown!!!")
        
    def lookup(self,points=None,logrho=None,logT=None):
        """A generic lookup function for our interpolator"""
        if points is None:
